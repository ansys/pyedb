==============
Create sources
==============

.. grid:: 2

   .. grid-item-card:: Circuit ports
            :link: create_circuit_ports_on_component
            :link-type: doc


   .. grid-item-card:: Coaxial ports
            :link: create_coax_port_on_component
            :link-type: doc
            :margin: 2 2 0 0


   .. grid-item-card:: Current sources
            :link: create_current_source
            :link-type: doc


   .. grid-item-card:: Edge ports
            :link: create_edge_port_on_polygon
            :link-type: doc
            :margin: 2 2 0 0


   .. grid-item-card:: Various ports
            :link: create_various_ports
            :link-type: doc


   .. grid-item-card:: Ports between pins and layers
            :link: create_port_between_pin_and_layer
            :link-type: doc
            :margin: 2 2 0 0


.. toctree::
   :hidden:
   :maxdepth: 2

   create_circuit_ports_on_component
   create_coax_port_on_component
   create_current_source
   create_edge_port_on_polygon
   create_various_ports
   create_port_between_pin_and_layer
