...
In PyEDB gRPC, source and port excitation workflows are centralized through the
``SourceExcitation`` class, which is exposed from ``Edb`` as
``edb.excitation_manager``.
...
``Siwave``. Instead, they are grouped into a dedicated manager focused on ports,
sources, and excitation objects.
...
.. note::
   ...
   ``edb.source_excitation`` still exists, but in the gRPC API it is marked as
   deprecated in favor of ``edb.excitation_manager``.
...
excitations. When documenting the gRPC architecture, it is therefore clearer to
present ``SourceExcitation`` as the main class for these workflows.
...
.. note::
   ...
   compatibility, such as ``hfss_setups``, ``siwave_dc_setups``, and
   ``siwave_ac_setups``. In the gRPC architecture, the preferred entry point is
   ``edb.simulation_setups``.
