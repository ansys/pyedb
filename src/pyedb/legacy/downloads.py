...
def download_aedb(destination=None):
    ...
    >>> import legacy
    >>> path = legacy.downloads.download_aedb()
    >>> path
    'C:/Users/user/AppData/local/temp/Galileo.aedb'
    """
    local_paths = []
    _download_file("legacy/edb/Galileo.aedb", "GRM32ER72A225KA35_25C_0V.sp", destination, local_paths)
    _download_file("legacy/edb/Galileo.aedb", "edb.def", destination, local_paths)
    return local_paths[-1]
...
def download_edb_merge_utility(force_download=False, destination=None):
    ...
    Download an example result file and return the path of the file.
    >>> import legacy
    >>> path = legacy.downloads.download_edb_merge_utility(force_download=True)
    >>> path
    ...
    local_paths = []
    _download_file("legacy/wpf_edb_merge/board.aedb", "edb.def", destination, local_paths)
    _download_file("legacy/wpf_edb_merge/package.aedb", "edb.def", destination, local_paths)
    _download_file("legacy/wpf_edb_merge", "merge_wizard_settings.json", destination, local_paths)

    _download_file("legacy/wpf_edb_merge", "merge_wizard.py", destination, local_paths)
...
def download_antenna_array(destination=None):
    ...
    local_paths = []
    _download_file("legacy/array_antenna", "FiniteArray_Radome_77GHz_3D_CADDM.aedt", destination, local_paths)
    return local_paths[0]
...
def download_sbr(destination=None):
    ...
    local_paths = []
    _download_file("legacy/sbr", "Cassegrain.aedt", destination, local_paths)
    return local_paths[0]
...
def download_icepak_3d_component(destination=None):  # pragma: no cover
    ...
    _download_file("legacy/icepak_3dcomp", "PCBAssembly.aedt", destination, local_paths)
...
def download_sherlock(destination=None):
    ...
    >>> import legacy
    >>> path = legacy.downloads.download_sherlock()
    >>> path
    'C:/Users/user/AppData/local/temp/Galileo.aedb'
    """
    if not destination:
        destination = EXAMPLES_PATH
    local_paths = []
    _download_file("legacy/sherlock", "MaterialExport.csv", destination, local_paths)
    _download_file("legacy/sherlock", "TutorialBoardPartsList.csv", destination, local_paths)
    _download_file("legacy/sherlock", "SherlockTutorial.aedt", destination, local_paths)
    _download_file("legacy/sherlock", "TutorialBoard.stp", destination, local_paths)
    _download_file("legacy/sherlock/SherlockTutorial.aedb", "edb.def", destination, local_paths)
...
def download_leaf(destination=None):
    ...
    >>> import legacy
    >>> path = legacy.downloads.download_leaf(r"c:\temp")
    >>> path
    ...
    if not destination:
        destination = EXAMPLES_PATH
    local_paths = []
    _download_file("legacy/nissan", "30DH_20C_smooth.tab", destination, local_paths)
    _download_file("legacy/nissan", "BH_Arnold_Magnetics_N30UH_80C.tab", destination, local_paths)
...
def download_custom_reports(force_download=False, destination=None):
    ...
    Download an example result file and return the path of the file.
    >>> import legacy
    >>> path = legacy.downloads.download_custom_reports(force_download=True)
    >>> path
    ...
    if force_download:
        ...
        if os.path.exists(local_path):
            shutil.rmtree(local_path, ignore_errors=True)
    download_file("legacy/custom_reports", destination=destination)
...
def download_3dcomponent(force_download=False, destination=None):
    ...
    Download an example result file and return the path of the file.
    >>> import legacy
    >>> path = legacy.downloads.download_3dcomponent(force_download=True)
    >>> path
    ...
    if force_download:
        ...
        if os.path.exists(local_path):
            shutil.rmtree(local_path, ignore_errors=True)
    download_file("legacy/array_3d_component", destination=destination)
...
def download_FSS_3dcomponent(force_download=False, destination=None):
    ...
    Download an example result file and return the path of the file.
    >>> import legacy
    >>> path = legacy.downloads.download_FSS_3dcomponent(force_download=True)
    >>> path
    'C:/Users/user/AppData/local/temp/array_3d_component'
    """
    ...
    if force_download:  # pragma: no cover
        ...
        if os.path.exists(local_path):  # pragma: no cover
            shutil.rmtree(local_path, ignore_errors=True)
    download_file("legacy/fss_3d_component", destination=destination)
...
def download_multiparts(destination=None):
    ...
    >>> import legacy
    >>> path = legacy.downloads.download_multiparts()
    >>> path
    ...
    _download_file("legacy/multiparts", "library.zip", destination)
...
def download_twin_builder_data(file_name, force_download=False, destination=None):
    ...
    Download an example result file and return the path of the file.
    >>> import legacy
    >>> path = legacy.downloads.download_twin_builder_data(file_name="Example1.zip",force_download=True)
    >>> path
    ...
    _download_file("legacy/twin_builder", file_name, destination)
