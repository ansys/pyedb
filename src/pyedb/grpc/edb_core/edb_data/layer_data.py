...
class LayerEdbClass(object):
    ...
    def is_stackup_layer(self):
        ...
        """
        return self._edb_layer.is_stackup_layer
    ...
    def is_via_layer(self):
        ...
        """
        return self._edb_layer.is_via_layer
    ...
    def transparency(self, transparency=0):
        layer_clone = self._edb_layer
        layer_clone.transparency = transparency
        self._pclass._set_layout_stackup(layer_clone, "change_attribute")
    ...
    def type(self, new_type):
        ...
        layer_clone = self._edb_layer
        if new_type == "signal":
            layer_clone.type = self._edb.cell.layer_type.SIGNAL_Layer
            self._type = new_type
        elif new_type == "dielectric":
            layer_clone.tyep = self._edb.cell.layer_type.DIELECTRIC_LAYER
            self._type = new_type
        else:
            return
        self._pclass._set_layout_stackup(layer_clone, "change_attribute")
...
class StackupLayerEdbClass(LayerEdbClass):
    ...
    def top_hallhuray_nodule_radius(self):
        ...
        if top_roughness_model:
            self._top_hallhuray_nodule_radius = top_roughness_model[0]
    ...
    def top_hallhuray_surface_ratio(self):
        ...
        top_roughness_model = self.get_roughness_model("top")
        if top_roughness_model:
            self._top_hallhuray_surface_ratio = top_roughness_model[1]
    ...
    def bottom_hallhuray_nodule_radius(self):
        ...
        if bottom_roughness_model:
            self._bottom_hallhuray_nodule_radius = bottom_roughness_model[0]
    ...
    def bottom_hallhuray_surface_ratio(self):
        ...
        bottom_roughness_model = self.get_roughness_model("bottom")
        if bottom_roughness_model:
            self._bottom_hallhuray_surface_ratio = bottom_roughness_model[1]
    ...
    def side_hallhuray_nodule_radius(self):
        ...
        if side_roughness_model:
            self._side_hallhuray_nodule_radius = side_roughness_model[0]
    ...
    def side_hallhuray_surface_ratio(self):
        ...
        side_roughness_model = self.get_roughness_model("side")
        if side_roughness_model:
            self._side_hallhuray_surface_ratio = side_roughness_model[1]
    ...
    def get_roughness_model(self, surface="top"):
        ...
        if not self.is_stackup_layer:  # pragma: no cover
            return
        if surface == "top":
            return self._edb_layer.get_roughness_model(self._pclass._pedb.edb_api.Layers.RoughnessRegion.TOP)
        elif surface == "bottom":
            return self._edb_layer.get_roughness_model(self._pclass._pedb.edb_api.Layers.RoughnessRegion.BOTTOM)
        elif surface == "side":
            return self._edb_layer.get_roughness_model(self._pclass._pedb.edb_api.Layers.RoughnessRegion.SIDE)
